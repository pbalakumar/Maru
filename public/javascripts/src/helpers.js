...
/*!
 * @description Smarty style templating for underscore
 */
...
/*!
 * @plugin .validate()
 * @description Form Validation tailored for bootstrap/plastique notifications
 */
...
(function($) {
  ...
  $.extend($.fn, {
    ...
    tableRowsToJSONWithFilter : function (filter, requiredField) {
      var re, item, attr, data, tableSelector = this, _JSON = [];
      // iterate over rows
      $('tr', tableSelector).each(function(index, tr) {
        // row to push onto map
        item = {};
        // collect tds into mapped attr/data values
        $('td', $(tr)).each(function(index, td) {
          attr = $(td).attr('data-id');
          data = $(td).text();
          // check for filters and validity
          if (attr !== undefined && data !== '' && new RegExp(filter, 'i').test(attr)) {
            re = new RegExp(filter + '\.(.*)');
            attr = attr.match(re)[1];
            item[attr] = data;
          };
        });
