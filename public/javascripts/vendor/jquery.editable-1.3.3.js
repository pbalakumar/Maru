(function ($) {
  ...
  $.fn.editable = function (options) {
    ...
    if (options == 'disable') return this.unbind(this.data('editable.options').editBy, this.data('editable.options').toEditable);
    if (options == 'enable') return this.bind(this.data('editable.options').editBy, this.data('editable.options').toEditable);
    if (options == 'destroy') return this.unbind(this.data('editable.options').editBy, this.data('editable.options').toEditable)
    ...
    options.toEditable = function () {
      ...
      } else $this.one(opts.submitBy, function () {
      ...
      // Cancel Event
      if (opts.cancel) $('<button/>').appendTo($this)
        .html(opts.cancel)
        .one('mouseup', function () {
        opts.toNonEditable($(this).parent(), false)
      });
      // Call User Function
      if ($.isFunction(opts.onEdit)) opts.onEdit.apply($this, [{
  ...
  $.editableFactory = {
    ...
    'checkbox':{
      toEditable: function ($this, options) {
        var checkbox = $('<input type="checkbox" />').appendTo($this);
        var currentValue = $this.data('editable.current');
        if (currentValue === '' || currentValue === 'NO') {
          $this.data('editable.current', 'YES');
          $this.attr('checked', 'checked');
        } else if (currentValue === 'YES') {
          $this.data('editable.current', 'NO');
          $this.removeAttr('checked');
        }
      },
      getValue: function ($this, options) {
        return ($this.data('editable.current') === 'YES') ? 'YES' : 'NO';
      }
    }
  }
})(jQuery);